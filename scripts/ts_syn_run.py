...
if __name__ == "__main__":
    ...
    syn_mcmm_file(
        os.path.join(TsGlobals.TS_SYN_RUN_DIR, TsGlobals.TS_SYN_MCMM_FILE), args
    )
    ...
    if (
        args.release
        and TsGlobals.TS_DESIGN_CFG["design"]["flow_dirs"]["syn"]
        and not args.force
    ):
        TsGlobals.TS_SYN_RELEASE_DIR = os.path.join(
            ts_get_root_rel_path(
                TsGlobals.TS_DESIGN_CFG["design"]["flow_dirs"]["syn"], args.runcode
            )
        )
