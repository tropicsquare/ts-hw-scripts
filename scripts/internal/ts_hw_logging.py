...
class TsErrCode(LogEnum):
    ...
    ERR_PDK_18 = [lambda spef, mode: "SPEF file '{}' for mode '{}' not found".format(spef, mode)]
    ERR_PDK_19 = [lambda dir, path: "Specified flow directory {}: {} not found".format(dir,path)]
    ERR_PDK_20 = [lambda file: "Specified map file {} does not exist".format(file)]
    ERR_PDK_21 = [lambda tluplus, mode: "Tlu+ file '{}' for mode '{}' not found".format(tluplus, mode)]
    ERR_PDK_22 = [lambda file: "Specified tech file {} does not exist".format(file)]
    ERR_PDK_23 = [lambda opcond,file: "Specified {} operation condition does not match with the {} file contant".format(opcond,file)]
    ERR_PDK_24 = [lambda file: "Design config file '{}' not found, can't continue!".format(file)]

    ERR_PDK_25 = [lambda name, version, view: "'{}({})' is missing view '{}', but the view is required".format(name, version, view)]


    # Power error messages
    ERR_PWR_0 = [lambda e: "Power config file is invalid.\n {}".format(e)]
    ERR_PWR_1 = [lambda: "Power config file not found. \n {}"]
    ERR_PWR_2 = [lambda e, pwr_file: "Failed to load power config file '{}'. \n {}".format(pwr_file, e)]
    ERR_PWR_3 = [lambda scenario, scenarios: "Invalid scenario {}!"
    ...
    ERR_MMAP_0 = [lambda e: "Key error: Sub-block '{}' should contain exactly one of (Key->'reg_map' or Key->'regions')".format(e)]
    ERR_MMAP_1 = [lambda e: "Missing key: File '{}' has no Key->'regions' defined at top most level".format(e)]
    ERR_MMAP_2 = [lambda e: "Invalid file: File '{}' is not a valid YAML file".format(e)]
    ...
    ERR_SYN_1 = [lambda runcode: "Cannot open {} runcode directory. It doesn't exist".format(runcode)]
    ERR_SYN_2 = [lambda runcode: "Directory runcode {} already exists. Use --force swith if you want to re-write it".format(runcode)]

    ERR_STA_0 = [lambda : "Missing runcode parameter"]
    ERR_STA_1 = [lambda runcode: "Cannot open {} runcode directory. It doesn't exist".format(runcode)]
    ERR_STA_2 = [lambda runcode: "Directory runcode {} already exists. Use --force swith if you want to re-write it".format(runcode)]
    ERR_STA_3 = [lambda source : "Source --source {} is not matching flow_dirs".format(source)]
...
class TsWarnCode(LogEnum):
    ...
    WARN_PDK_6 = [lambda: "No views added, skipping TCL export. Use '--add-views=<views>' to define which views you would like to export"]
    ...
    # PWR warnings
    WARN_PWR_0 = [lambda scenario: "Scenario '{}' is not randomized, so it will only run once!".format(scenario)]
    WARN_PWR_1 = [lambda: ""]
    WARN_PWR_2 = [lambda: ""]
...
class TsInfoCode(LogEnum):
    ...
    # Memory map generator error messages
    INFO_MMAP_0 = [lambda start_addr, end_addr: "with start address: {} and end address: {}".format(start_addr, end_addr)]
    ...
    INFO_SYS_2 = [lambda runcode: "Deleting and creating new synthesis database: {}".format(runcode)]
    INFO_SYS_3 = [lambda tool_type, cmd: "Following {} cmd to be executed: {}".format(tool_type,cmd)]
    ...
    INFO_STA_2 = [lambda runcode: "Deleting and creating new sta database: {}".format(runcode)]
    INFO_STA_3 = [lambda tool_type, cmd: "Following {} cmd to be executed: {}".format(tool_type,cmd)]
