...
class GRAMMAR_MEM_MAP_CONFIG:
    ...
    def validate(cls, value:dict): 
        if value['start_addr'] < 0: 
            ts_throw_error(TsErrCode.GENERIC, f"Region: {value['name']} has a negative start address defined ({hex(value['start_addr'])})")        

        if value['end_addr'] < 0: 
            ts_throw_error(TsErrCode.GENERIC, f"Region: {value['name']} has a negative end address defined ({hex(value['end_addr'])})")        


        if value['start_addr'] == value ['end_addr']:
            ts_print(f"WARNING: Region: {value['name']} has the same start and end addresses: {value['start_addr']}", color=TsColors.RED)        

        elif value['start_addr'] > value ['end_addr']:
            ts_throw_error(TsErrCode.GENERIC, f"Region: {value['name']} has start address ({value['start_addr']}) greater than end address ({value['end_addr']})")
