...
def __load_sim_config_file(sim_cfg_path):
    ...
    try:
        with open(sim_cfg_path) as cfg_file:
            cfg = yaml.safe_load(cfg_file)
    except FileNotFoundError:
        ts_throw_error(TsErrCode.ERR_CFG_0)
    except:
        ts_throw_error(TsErrCode.ERR_CFG_1)
...
def __merge_args_with_config(args):
    ...
    for attr_key in GRAMMAR_SIM_CFG:
        ...
        # If destination attribute does not exist in the config, then set it! This allows to
        # set things like "verbose" even if they are not present in config file!
        if attr_key not in ts_get_cfg():
            ts_set_cfg(attr_key, attr_val)
        # Over-ride attribute only if True. The basic flow is that if something is enabled in
        # simulation config file, you can't disable it by command line
...
def __finalize_config():
    ...
    """
    ts_debug("Expanding environment variables")

    def _expand_vars(cfg):
        if isinstance(cfg, dict):
            return {k: _expand_vars(v) for k, v in cfg.items()}
        elif isinstance(cfg, list):
            return [_expand_vars(e) for e in cfg]
        elif isinstance(cfg, str):
            return os.path.expandvars(cfg)
        return cfg

    TsGlobals.TS_SIM_CFG = _expand_vars(TsGlobals.TS_SIM_CFG)
    ...
    if "targets" not in TsGlobals.TS_SIM_CFG:
        ts_throw_error(TsErrCode.ERR_GENERIC, "'targets' key is missing from configuration!")
    for target in TsGlobals.TS_SIM_CFG["targets"]:
...
def do_config_init(args):
    ...
    # We can't use "ts_is_verbose" here, command line args are not merged with config file yet!
    ...
    ts_debug("Simulation config file:")
    ts_debug(str(ts_get_cfg()))

    # Merging command line arguments with config file
    __merge_args_with_config(args)
    if args.very_verbose:
        ts_info(TsInfoCode.INFO_GENERIC, "Simulation configuration (merged config file and "
                                         "command-line arguments):")
        ts_info(TsInfoCode.INFO_GENERIC, str(ts_get_cfg()))

    # Finalize configuration
    __finalize_config()

    # Checking simulation config file
    # Note: Here arguments are already merged, we can use "ts_is_verbose"
...
def print_target_list():
    ...
    """
    ts_info(TsInfoCode.INFO_CMN_21)
    for sim_target in ts_get_cfg("targets"):
        print("     {}".format(sim_target))
