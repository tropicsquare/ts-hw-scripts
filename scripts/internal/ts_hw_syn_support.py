...
def set_syn_global_vars(args):
    ...
    TsGlobals.TS_SYN_LOGS_DIR = os.path.join(
        TsGlobals.TS_SYN_RUN_DIR, TsGlobals.TS_SYN_LOGS_DIR
    )
    TsGlobals.TS_SYN_RESULTS_DIR = os.path.join(
        TsGlobals.TS_SYN_RUN_DIR, TsGlobals.TS_SYN_RESULTS_DIR
    )
    TsGlobals.TS_SYN_REPORTS_DIR = os.path.join(
        TsGlobals.TS_SYN_RUN_DIR, TsGlobals.TS_SYN_REPORTS_DIR
    )
    # Setting paths for open synthesis database
    TsGlobals.TS_SYN_DC_RM_OPENFILE = os.path.join(
        TsGlobals.TS_SYN_RUN_DIR, TsGlobals.TS_SYN_DC_RM_OPENFILE
    )
    # Setting path for run sythesis
    TsGlobals.TS_SYN_DC_RM_RUNFILE = os.path.join(
        get_env_var_path(TsGlobals.TS_SYN_FLOW_PATH), TsGlobals.TS_SYN_DC_RM_RUNFILE
    )
    ...
    TsGlobals.TS_SYN_DESIGN_CFG_FILE = os.path.join(
        TsGlobals.TS_SYN_RUN_DIR, TsGlobals.TS_SYN_DESIGN_CFG_FILE
    )
    # Setting mcmm setup file path and name
    TsGlobals.TS_SYN_MCMM_FILE = os.path.join(
        TsGlobals.TS_SYN_RUN_DIR, TsGlobals.TS_SYN_MCMM_FILE
    )
...
def syn_mcmm_file(path: str, args):
    ...
    lines.append(
        f"     set_operating_conditions -max [dict get $OPCOND [dict get $MODES $MODE corner]]\n"
    )
    lines.append(
        f"     set_tlu_plus_files -max_tluplus [dict get $MODES $MODE tluplus] -tech2itf_map $MW_MAP_FILE \n"
    )
    lines.append(
        f"     source -echo -verbose -continue_on_error [dict get $MODES $MODE constraints]\n"
    )
    lines.append(f"}}\n")
    lines.append(f"\n")
...
def syn_setup(path: str, args):
    ...
    lines.append(f"\n")
    lines.append(
        f"# TODO: Remove indirect pass and pass TS_MILKYWAY_DIRS to main synthesis script.\n"
    )
    ...
    lines.append(f"#\n")
    lines.append(
        f"# For debug, it is sometimes necessary to stop after some of these stesp and go back to\n"
    )
    lines.append(
        f"# TCL command line of the synthesis tool. Following variables configure this:\n"
    )
    lines.append(f"\n")
    lines.append(f"set BREAK_AFTER_LINK         {str(args.break_after_link).lower()}\n")
    lines.append(
        f"set BREAK_AFTER_CONSTRAINTS  {str(args.break_after_constraints).lower()}\n"
    )
    lines.append(
        f"set BREAK_AFTER_COMPILE      {str(args.break_after_compile).lower()}\n"
    )
    lines.append(
        f"set BREAK_AFTER_REPORTS      {str(args.break_after_reports).lower()}\n"
    )
    lines.append(f"\n")
    lines.append(f"unset MODES\n")
