...
def build_sta_cmd(args):
    ...
    pt_cfg_args += f'set REPORT_POSTPROC "false"; set RUNCODE_CMD_LINE {TsGlobals.TS_STA_RUNCODE};'
...
def sta_dmsa_file(path: str, args):
    ...
    lines.append(f'\n')
    lines.append(f'     create_scenario -mode [dict get $MODES $MODE name] -corner [dict get $MODES $MODE corner] -common_variables {{MODE SDC_EXPORT STAYIN RUNCODE_CMD_LINE SIGNOFF REPORTS_DIR RESULTS_DIR DMSA}} -common_data {TsGlobals.TS_STA_DC_RM_RUNFILE}\n')
    lines.append(f'}}\n')
    lines.append(f'\n')
...
def sta_setup(path: str, args):
    ...
    lines.append(f'\n')
    lines.append(f'set ADDITIONAL_SEARCH_PATH [concat ${{TS_NLDM_DB_VIEW_DIRS}} ${{TS_CCS_DB_VIEW_DIRS}}] \n')
    lines.append(f'\n')
    lines.append(f'\n')
    ...
    lines.append(f'\n')
    #lines.append(f'foreach MODE [dict keys $MODES] {{\n')
    lines.append(f' set TARGET_LIBRARY_FILES_STA [concat [set TARGET_LIBRARY_FILES_STA] [set TS_NLDM_DB_[set MODE]_VIEWS]] \n')
    #lines.append(f'}}\n')
    lines.append(f'\n')
    ...
    lines.append(f'set DCRM_CONSTRAINTS_INPUT_FILE [set TS_{str(ts_get_design_top())}_[set MODE]_CONSTRAINTS] \n')
    lines.append(f'\n')
    ...
    if args.sign_off:
        ...
        lines.append(f'\n')
        lines.append(f'set PARASITIC_PATHS {ts_get_root_rel_path(TsGlobals.TS_DESIGN_CFG["design"]["flow_dirs"][args.source_data])}/{TsGlobals.TS_STA_RUNCODE}/results/ \n')
        lines.append(f'\n')
    lines.append(f'\n')
    lines.append(f'\n')
    lines.append(f'\n')
...
def sta_open_design(args):
    ...
    for i, mode in enumerate(TsGlobals.TS_DESIGN_CFG["design"]["modes"]):
        if args.mode in mode["name"]:
            lines.append(f'\n')
            lines.append(f'restore_session ./{mode["name"]}_{mode["corner"]}/{str(ts_get_design_top()).lower()}_ss \n')
            lines.append(f'\n')
