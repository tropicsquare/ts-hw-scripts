...
def __add_generics(cfg_dict: dict) -> list:
    ...
    if simulator == "vcs":
        ...
        for gen_name, gen_val in cfg_dict["generics"].items():
            ...
            # Expand environment variable and convert to int if needed
            with contextlib.suppress(TypeError):
                gen_val = os.path.expandvars(gen_val)
                if gen_val.isdecimal():
                    gen_val = int(gen_val)
...
def __add_parameters(cfg_dict: dict) -> list:
    ...
    if simulator == "vcs":
        ...
        for param_name, param_val in cfg_dict["parameters"].items():
            ...
            # Expand environment variable and convert to int if needed
            with contextlib.suppress(TypeError):
                param_val = os.path.expandvars(param_val)
                if param_val.isdecimal():
...
def __build_compile_command(language: str, sim_cmds_dict: dict,
    ...
    return " ".join(collections.OrderedDict.fromkeys(filter(str.strip, comp_cmd)))
...
def __compile_all_files():
    ...
    def _get_included_files(*elements):
        ...
        return list(itertools.chain.from_iterable(
                    map(lambda d:
                        map(lambda f: os.path.join(d, f),
                            filter(lambda f: os.path.isfile(f) and not os.path.islink(f)
                                        and os.path.splitext(f)[1] in (".v", ".sv", ".svh", ".svi", ".pkg"),
                                    os.scandir(d))),
                        included_dirs)))
    ...
    for lib, source_files in TsGlobals.TS_SIM_SRCS_BY_LIB.items():
        ...
        for source_file_dict in source_files:
            ...
            # Print the command
            ts_info(TsInfoCode.INFO_CMN_22, source_file_dict["full_path"])
...
def __build_elab_command(test: dict, top_entity: str, log_file_path: str,
    ...
    return " ".join(collections.OrderedDict.fromkeys(filter(str.strip, elab_cmd)))
...
def __build_sim_command(sim_binary: str, sim_cmds: dict, test: dict, log_file_path: str,
    ...
    return " ".join(collections.OrderedDict.fromkeys(filter(str.strip, sim_cmd)))
