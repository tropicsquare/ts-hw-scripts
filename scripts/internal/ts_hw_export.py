...
def __append_to_view_tcl_lists(view_list, folder_list, view_val):
    ...
    if type(view_val) == list:
        view_list_ext = ["    {} \\\n".format(os.path.basename(x)) for x in view_val]
        folder_list_ext = ["    {} \\\n".format(os.path.dirname(x)) for x in view_val]
    else:
...
def __export_rtl_build_dirs(fd):
    ...
    for lib in TsGlobals.TS_SIM_SRCS_BY_LIB.keys():
        build_dirs.append("    $TS_SYNTH_DIR/{}\\\n".format(lib))

    fd.write("# RTL build directories for synthesis\n")
