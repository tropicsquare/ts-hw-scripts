        ...
        if obj_type in TsGlobals.TS_DESIGN_CFG["design"]:
            ...
            for target_obj in val:
                ...
                if view not in obj["views"]:
                    if (enforce):
                        ts_throw_error(TsErrCode.ERR_PDK_25, target_obj_name, target_obj_version, view)
                    else:
...
def __export_wireload(fd):
    ...
    if "wireload" not in TsGlobals.TS_DESIGN_CFG["design"]:
        ...
        constr_var_part += f"# RC corners \n"
        constr_var_part += f"{'#'*85}\n\n"
        constr_dict_part[TsGlobals.TS_DESIGN_CFG["design"]["wireload"]["library"]] = TsGlobals.TS_DESIGN_CFG["design"]["wireload"]["name"]
        fd.write(constr_var_part)
        __write_tcl_dict("TS_{}_WIRELOAD".format(ts_get_design_top()), constr_dict_part, fd)
...
def __export_map_tech(fd):
    ...
    if "map" not in TsGlobals.TS_DESIGN_CFG["design"]:
        ...
        fd.write("set TS_{}_TECH {}\n".format(ts_get_design_top().upper(), TsGlobals.TS_DESIGN_CFG["design"]["tech"]))
        fd.write("\n")
...
def __export_flow_dirs(fd):
    ...
    if "flow_dirs" not in TsGlobals.TS_DESIGN_CFG["design"]:
        ...
        for key in TsGlobals.TS_DESIGN_CFG["design"]["flow_dirs"].keys():
            fd.write(f"set TS_{str(key).upper()}_FLOW_EXPORT_DIR {ts_get_root_rel_path(TsGlobals.TS_DESIGN_CFG['design']['flow_dirs'][key])}\n")
        fd.write("\n")
