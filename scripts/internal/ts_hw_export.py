...
def __append_do_not_modify(lines: list):
    ...
    lines.append("#")
    lines.append(
        "# THIS FILE IS AUTO-GENERATED. DO NOT MODIFY IT (UNLESS DOING LOCAL DEBUG)!"
    )
    lines.append("#")
...
def __get_syn_rtl_header(tool: str) -> list:
    ...
    lines.append("#")
    lines.append(
        f"# This script is exported by ts-hw-scripts and it shall be sourced by {tool} to load"
    )
    lines.append(
        f"# RTL source files from target '{TsGlobals.TS_SIM_CFG['target']}' to dc_shell."
    )
    lines.append("#")
    if tool == "dc_shell":
        ...
        lines.append(
            "# Sourcing this file sets variable TS_VIVADO_DEFINES and TS_VIVADO_INCDIRS which gather"
        )
        lines.append(
            "# Verilog defines and include directories. These variables shall be used with 'synth_design'"
        )
...
def __get_design_config_header(views: list, args) -> list:
    ...
    lines.append("#")
    lines.append(
        "# This script is exported by ts-hw-scripts and it shall be sourced by a CAD tool to"
    )
    lines.append(
        "# load PDK views into TCL shell variables. Export is intentionally exporting view"
    )
    lines.append(
        "# paths to TCL variables only, not tool specific variables (e.g. 'link_library'). "
    )
    lines.append("#")
...
def export_dc_tcl(tcl_file: str):
    ...
    for lib in TsGlobals.TS_SIM_SRCS_BY_LIB:
        ...
        lines.append(
            f"define_design_lib {lib} -path {TsGlobals.TS_SYN_BUILD_DIR}/{lib}\n"
        )
    ...
    for lib, source_files in TsGlobals.TS_SIM_SRCS_BY_LIB.items():
        ...
        for source_file in source_files:
            ...
            if lang != "vhdl":
                ...
                defines = [
                    f"{name}={val}" if val is not None else name
    ...
    try:
        hdl_lib, top_entity = TsGlobals.TS_SIM_CFG["targets"][
            TsGlobals.TS_SIM_CFG["target"]
        ]["top_entity"].split(".")
        lines.append(f"elaborate -work {hdl_lib} {top_entity}\n")
    except ValueError:
        lines.append(
            f"elaborate {TsGlobals.TS_SIM_CFG['targets'][TsGlobals.TS_SIM_CFG['target']]['top_entity']}\n"
        )
...
def export_vivado_tcl(tcl_file: str):
    ...
    def __add_included_dir(lst, include_dir):
        ...
        lst.append(
            f'append TS_VIVADO_INC_DIRS "{ts_get_root_rel_path(include_dir)} "\n'
        )
...
def __export_opcond_with_corners(fd):
    """
    Exports only operation condition according to std_cells opcond definition.
    """
    fd.write("#" * 85 + "\n")
    fd.write("# Operation conditions\n")
    fd.write("#" * 85 + "\n\n")

    per_obj_dict = {}
    if "std_cells" in TsGlobals.TS_DESIGN_CFG["design"]:
        val = TsGlobals.TS_DESIGN_CFG["design"]["std_cells"]
        # If there is more than a single std_cell definition
        for target_obj in val:
            assert type(target_obj) == dict
            target_obj_name = list(target_obj.keys())[0]
            target_obj_version = list(target_obj.values())[0]
            obj = get_pdk_obj("std_cells", target_obj_name, target_obj_version)
            for mode in TsGlobals.TS_DESIGN_CFG["design"]["modes"]:
                if mode["corner"] in get_pdk_corners().keys():
                    per_obj_dict[mode["corner"]] = obj["opcond"][mode["corner"]]

    __write_tcl_dict("TS_{}_OPCOND".format(ts_get_design_top()), per_obj_dict, fd)
...
def __export_rtl_build_dirs(fd):
    ...
    __write_tcl_list(
        "TS_{}_RTL_SYN_BUILD_DIRS".format(ts_get_design_top()), build_dirs, fd
    )
...
def __export_constraints(fd):
    ...
    if "constraints" in TsGlobals.TS_DESIGN_CFG["design"]:
        ...
        __write_tcl_list(
            "TS_{}_GLOBAL_CONSTRAINTS".format(ts_get_design_top()), constr_list, fd
        )
    ...
    if constr_dict:
        fd.write("\n")
        __write_tcl_dict(
            "TS_{}_CONSTRAINTS".format(ts_get_design_top()), constr_dict, fd
        )
...
def __export_spef(fd):
    ...
    if constr_dict_part != {}:
        fd.write("\n")
        __write_tcl_dict(
            "TS_{}_SPEFS".format(ts_get_design_top()), constr_dict_part, fd
        )
    else:
...
def __export_rc_corner(fd):
    ...
    if constr_dict_part != {}:
        __write_tcl_dict(
            "TS_{}_RC_CORNERS".format(ts_get_design_top()), constr_dict_part, fd
        )
    else:
...
def __export_floorplan(fd):
    ...
    if "floorplan" not in TsGlobals.TS_DESIGN_CFG["design"]:
        ...
        fd.write(
            "set TS_{}_FLOORPLAN {}\n".format(
                ts_get_design_top().upper(),
                TsGlobals.TS_DESIGN_CFG["design"]["floorplan"],
            )
        )
        fd.write("\n")


def __export_wireload(fd):
    """ """
    if "wireload" not in TsGlobals.TS_DESIGN_CFG["design"]:
        ts_warning(TsWarnCode.WARN_PDK_5)
    elif "name" not in TsGlobals.TS_DESIGN_CFG["design"]["wireload"]:
        ts_warning(TsWarnCode.WARN_PDK_5)
    elif "library" not in TsGlobals.TS_DESIGN_CFG["design"]["wireload"]:
        ts_warning(TsWarnCode.WARN_PDK_5)
    else:
        constr_dict_part = {}
        constr_var_part = f""
        ...
        constr_var_part += f"{'#'*85}\n\n"
        constr_dict_part[
            TsGlobals.TS_DESIGN_CFG["design"]["wireload"]["library"]
        ] = TsGlobals.TS_DESIGN_CFG["design"]["wireload"]["name"]
        fd.write(constr_var_part)
        __write_tcl_dict(
            "TS_{}_WIRELOAD".format(ts_get_design_top()), constr_dict_part, fd
        )


def __export_map_tech(fd):
    """ """
    if "map" not in TsGlobals.TS_DESIGN_CFG["design"]:
        ts_warning(TsWarnCode.WARN_PDK_7)
    elif "tech" not in TsGlobals.TS_DESIGN_CFG["design"]:
        ts_warning(TsWarnCode.WARN_PDK_7)
    else:
        fd.write("#" * 85 + "\n")
        fd.write("# Map and tech files\n")
        fd.write("#" * 85 + "\n\n")
        fd.write(
            "set TS_{}_MAP {}\n".format(
                ts_get_design_top().upper(), TsGlobals.TS_DESIGN_CFG["design"]["map"]
            )
        )
        fd.write(
            "set TS_{}_TECH {}\n".format(
                ts_get_design_top().upper(), TsGlobals.TS_DESIGN_CFG["design"]["tech"]
            )
        )
        fd.write("\n")
...
def __export_flow_dirs(fd):
    ...
    if "flow_dirs" not in TsGlobals.TS_DESIGN_CFG["design"]:
        ts_warning(TsWarnCode.WARN_PDK_11)
    else:
        for key in TsGlobals.TS_DESIGN_CFG["design"]["flow_dirs"].keys():
            fd.write(
                f"set TS_{str(key).upper()}_FLOW_EXPORT_DIR {ts_get_root_rel_path(TsGlobals.TS_DESIGN_CFG['design']['flow_dirs'][key])}\n"
            )
        fd.write("\n")


def export_design_config(path: str, args, enforce_views=[]):
    """ """
    fd = open(path, "w")
    __add_newline = lambda x: x + "\n"
    ...
    # write header
    fd.writelines(
        map(__add_newline, __get_design_config_header(TsGlobals.TS_EXP_VIEWS, args))
    )

    # Write design target (top entity)
    if args.add_top_entity:
        top_entity = ts_get_cfg()["targets"][
    ...
    for view in TsGlobals.TS_EXP_VIEWS:
        ...
        enforce = False
        if view in enforce_views:
            enforce = True

        if view_has_corner(view):
            __export_view_with_corners(view, fd, enforce)
        else:
            __export_view_no_corners(view, fd, enforce)
    ...
    # Export wireload
    if args.add_wireload:
        __export_wireload(fd)
    ...
    # Export map file
    if args.add_map_tech:
        __export_map_tech(fd)
    ...
    # Export operation conditions
    if args.add_opcond:
        __export_opcond_with_corners(fd)
