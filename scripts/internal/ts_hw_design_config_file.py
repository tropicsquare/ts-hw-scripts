...
def __check_flow_dirs_exists():
    ...
    for item in TsGlobals.TS_DESIGN_CFG["design"]["flow_dirs"]:
        for key in item:
            ...
            if os.path.exists(ts_get_root_rel_path(item[key])):
                ts_debug(f" Flow directory exists -{key}: {item[key]}")
            else: 
                ts_throw_error(TsErrCode.ERR_PDK_19,key,ts_get_root_rel_path(item[key]))
...
def __check_modes_valid():
    ...
    for i, mode in enumerate(TsGlobals.TS_DESIGN_CFG["design"]["modes"]):
        ...
        if "spef" in mode:
            if os.path.exists(ts_get_root_rel_path(mode["spef"])):
                TsGlobals.TS_DESIGN_CFG["design"]["modes"][i]["spef"] = ts_get_root_rel_path(mode["spef"])
            else:
                ts_throw_error(TsErrCode.ERR_PDK_18, TsGlobals.TS_DESIGN_CFG["design"]["modes"][i]["spef"],mode["name"])
