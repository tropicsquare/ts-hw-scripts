...
def __check_ignore_uvm(line: str):
    ...
    if re.search(BUILT_IN_UVM_IGNORE_START_PATTERN, line):
        #print("Starting UVM ignore on line: {}", line)
        __UVM_REPORT_SUMMARY = True
    
    if re.search(BUILT_IN_UVM_IGNORE_STOP_PATTERN, line):
        #print("Stopping UVM ignore on line: {}", line)
        __UVM_REPORT_SUMMARY = False
...
def __to_text_result(test_results: dict):
    """
    Convert test result to textual representation (colorized PASS/FAIL)
    :param test_results: Test results object of a single test.
    """
    str_result = [[f"{CRED}FAIL{CEND}", "FAIL"],
                    [f"{CGREEN}PASS{CEND}", "PASS"]]

    return str_result[test_results["result"]][ts_get_cfg("no_color")]


def __print_err_warn_lines(print_list: list):
    """
    Prints a list of errors/warning/ignored errors/ignored warnings.
    :param print_list: List to be printed.
    """
    for entry in print_list:
        line_title = "Line {}:".format(entry["line_number"])
        print("{} {}".format(line_title, entry["line"].rstrip()))
        for after_line in entry.get("after_lines", {}):
            print("{}{}".format(" " * (len(line_title) + 1), after_line["line"].rstrip()))
...
def print_test_result(test_result: list):
    ...
    if undefined_keys:
        ts_script_bug("{} key(s) shall be defined in test results.".format(undefined_keys))
    ...
    print(__RESULT_FORMAT.format(*map(str, (
                                    ...
                                    __to_text_result(test_result)))))

    if ts_is_at_least_verbose():
        ts_info(TsInfoCode.INFO_CMN_17)
        __print_err_warn_lines(test_result["errors"])

        ts_info(TsInfoCode.INFO_CMN_18)
        __print_err_warn_lines(test_result["warnings"])

        ts_info(TsInfoCode.INFO_CMN_19)
        __print_err_warn_lines(test_result["ignored_errors"])

        ts_info(TsInfoCode.INFO_CMN_20)
        __print_err_warn_lines(test_result["ignored_warnings"])
